...
createContainer :: ContainerSpec -> T.Text -> DockerClient (Map.Map T.Text Value)
createContainer cspec name = do
    cfg <- ask
    lift $ do
        let opts = defaults & param "name" .~ [name]
        rsp <- asJSON =<< postWith opts (apiBase cfg ++ "/containers/create") (toJSON cspec)
